...
import { DesignAuditResult, SuggestedIssue, GoalContractData } from '../types';
...
export const DesignAuditWorkspace: React.FC<DesignAuditWorkspaceProps> = React.memo(({
  result,
  isGeneratingAudit,
  auditError,
  onCopy,
  ...
  const getSeverityBadge = (sev: string) => {
    switch (sev) {
      ...
      case 'medium': return 'bg-amber-950/40 border border-amber-900/30 text-primary';
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center p-8 min-h-[500px]" id="design-generating-state">
        ...
        <div className="text-center max-w-sm">
          <h3 className="font-sans text-primary font-bold text-sm tracking-wider uppercase leading-snug">Performing Design Audit</h3>
      ...
      <div className="flex-1 flex flex-col p-6 animate-fade-in" id="design-error-state">
        <div className="bg-[#1F1414] border border-rose-950/30 p-4 rounded-xl flex items-start gap-3">
          ...
          <div className="flex-1">
            <h3 className="font-sans font-bold text-xs uppercase tracking-wider text-rose-300">Design Audit Failed</h3>
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden animate-fade-in" id="design-audit-workspace">
      ...
      <div className="px-5 py-4 bg-slate-950/20 border-b border-[#1F1F1F] flex flex-col sm:flex-row gap-4 items-center justify-between">
        <div>
          <h2 className="font-sans text-sm font-bold tracking-wider uppercase text-primary">Design Principles Audit Workspace</h2>
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto shrink-0 justify-end">
          ...
          <button
            ...
            onClick={() => {
              if (!result) return;
              onOpenGoalBuilder?.({
                title: `${result.projectName} - UI/UX FIXES`,
                objective: `Address the identified UI/UX and accessibility issues in the design audit for ${result.projectName}.\n\nOverall UX Score: ${result.overall_score}/10\n\nIdentified Issues:\n${result.issues.map(issue => `- [${issue.severity.toUpperCase()}] ${issue.category}: ${issue.problem}\n  Recommended Fix: ${issue.recommended_fix}`).join('\n')}\n\nQuick Wins:\n${result.quick_wins.map(w => `- ${w}`).join('\n')}`,
                includedAssets: '',
                verificationCommand: 'npm run build',
                successMetric: 'Build compiles successfully, and all modified components resolve visual linter and accessibility checks with zero errors.'
              });
            }}
      ...
      <div className="flex-1 p-5 overflow-y-auto scroller-custom bg-[#0A0A0A] flex flex-col gap-6 max-h-[820px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-5 items-stretch">
          ...
          <div className="md:col-span-4 bg-[#111111] border border-[#1F1F1F] rounded-xl p-5 flex flex-col items-center justify-center gap-3">
            ...
            <div className="relative h-28 w-28 flex items-center justify-center">
              {/* Outer Cybertech Cyan Ring design */}
              <div className="absolute inset-0 rounded-full border-4 border-[#262626]" />
              <div className="absolute inset-0 rounded-full border-4 border-transparent border-t-[#00e5ff] border-r-[#00e5ff] animate-spin-slow" />
              <div className="text-center z-10">
                <span className="text-3xl font-sans font-black tracking-wider text-primary block">
          ...
          <div className="md:col-span-8 bg-[#111111] border border-[#1F1F1F] rounded-xl p-5 flex flex-col gap-3 justify-center">
            <h3 className="text-[10px] font-mono tracking-widest font-bold text-primary uppercase">Audit Summary</h3>
        ...
        <div className="flex flex-col gap-2">
          <span className="text-[10px] font-mono tracking-widest font-bold text-slate-500 uppercase flex items-center gap-1.5">
            <Sliders className="h-4 w-4 text-primary" /> Dimension Rating Metrics
          </span>
            ...
            {Object.entries(result.scores).map(([key, scoreVal]) => {
              ...
              return (
                <div key={key} className="bg-[#111111] border border-[#1F1F1F] p-3 rounded-lg flex flex-col gap-1.5 text-center justify-between">
                  ...
                  <div>
                    <span className={`text-base font-sans font-extrabold tracking-wider uppercase block ${getRatingColor(score)}`}>
                      {score}
                    </span>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 gap-5">
            {result.issues.map((iss) => (
              <div key={iss.id} className="bg-[#111111] border border-[#1F1F1F] hover:border-primary/25 rounded-2xl overflow-hidden transition-all duration-300 flex flex-col">
                ...
                <div className="p-5 flex flex-col gap-3">
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500 mt-2 font-mono leading-relaxed leading-snug flex items-start gap-1">
                      <span className="text-primary">⚠️ Why it matters:</span> {iss.why_it_matters}
                    </p>
                  </div>
        ...
        <div className="bg-[#111111] border border-gradient-to-r border-primary/20 rounded-2xl overflow-hidden shadow-xl mt-2 flex flex-col">
          <div className="px-5 py-4 bg-[#1F1A14]/30 border-b border-[#1F1F1F] flex flex-col sm:flex-row gap-3 justify-between items-start sm:items-center">
            <div className="flex items-center gap-2">
              ...
              <div>
                <h3 className="text-xs font-bold font-mono uppercase text-primary tracking-wider">Master UI Implementation Prompt</h3>
            ...
            <button
              ...
              className="text-[10px] bg-[#161616] hover:bg-[#222222] text-primary border border-[#262626] px-3.5 py-2 rounded-xl transition inline-flex items-center gap-1.5 cursor-pointer font-bold font-mono uppercase tracking-wider justify-center w-full sm:w-auto"
            >
              {copied ? (
                <>
                ...
                <>
                  <Copy className="h-3.5 w-3.5 text-primary" /> Copy Master Prompt
                </>
              )}
            </button>
          </div>
